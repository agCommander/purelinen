        ...
        const proxyReq = http.request({
          ...
          proxyRes.on("data", (chunk) => {
            ...
            try {
              ...
              console.log("[Auth Route] Admin authentication successful, returning JWT token")
              console.log("[Auth Route] The admin SDK will call POST /auth/session with this token to create the session")
              
              // Simply forward the JWT token response - don't create session here
              // The admin SDK will call POST /auth/session with the JWT token
              // Our custom POST /auth/session route will handle creating the session
