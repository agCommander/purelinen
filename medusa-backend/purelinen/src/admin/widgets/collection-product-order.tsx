...
const CollectionProductOrderWidget = ({ data }: DetailWidgetProps<AdminCollection>) => {
  ...
  const fetchProducts = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/admin/custom/collections/${data.id}/products`);
      if (response.ok) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <Text className="text-small text-gray-500 mt-1">
            Set the display order of products in this collection
          </Text>
        </div>
        <Button
          variant="secondary"
      ...
      <Drawer open={isDrawerOpen} onOpenChange={setIsDrawerOpen}>
        <Drawer.Content className="max-w-2xl max-h-[90vh] flex flex-col">
          <Drawer.Header className="flex-shrink-0">
            <Drawer.Title>Product Order - {data.title}</Drawer.Title>
          </Drawer.Header>
          <Drawer.Body className="flex-1 overflow-y-auto p-4">
            {isLoading ? (
              <Text>Loading products...</Text>
            ) : products.length === 0 ? (
              <Text className="text-gray-500">No products in this collection.</Text>
            ) : (
...
export default CollectionProductOrderWidget;
